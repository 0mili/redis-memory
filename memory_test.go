...
func newTestMemory(t *testing.T) (mili.Memory, *redimock.Server, func()) {
...
func TestMemory(t *testing.T) {
	...
	store := mili.NewStorage(logger)
	miliConf := mili.NewConfig(logger, nil, store, nil)
	err = mod.Apply(&miliConf)
	require.NoError(t, err)
}
...
func TestNewMemory(t *testing.T) {
	...
	mem := m.(*memory)
	assert.Equal(t, "mili-bot", mem.hkey)
